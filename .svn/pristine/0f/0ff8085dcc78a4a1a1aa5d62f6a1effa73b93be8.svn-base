...
const mutations = {
	...
	jumpTo (state, direction) { // 全局跳转测试、正式替换
		...
		var locationArray = [{
			...
			name: '问医生',
			testLink: 'http://test-ddys-wechat.hztywl.cn/WeChat/new-consult/#/home',
			...
			name: '团队咨询',
			testLink: 'http://test-ddys-wechat.hztywl.cn/WeChat/consultTeamDoc/#/',
			officialLink: 'https://ddys-wechat.diandianys.com/WeChat/consultTeamDoc/#/',
			ddhyLink: 'https://ddys-wechat.diandianys.com/WeChat/consultTeamDoc/#/'
		}]
		for (var i in locationArray) {
			...
			if (options.name == locationArray[i].name) {
				...
				// 除测试以为全部跳正式地址
				if (ENV_CONFIG.wxid == 'wxe274576d295e0050') {
					a.href = locationArray[i].testLink
				} else if (ENV_CONFIG.wxid == 'wxe143c7c832718250') {
